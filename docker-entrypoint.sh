...
#
# Dispatch the single vgi-color image into one of its transports:
...
#
# The worker is stateless (pure formula engine + static named-color table), so
...
case "${1:-http}" in
  http)
    ...
    exec color-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec color-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec color-worker "$@"
