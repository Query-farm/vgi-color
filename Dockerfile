...
#
# Single image that serves every network transport of the `color` VGI worker:
...
#
# Like vgi-units this worker is STATELESS: the color engine implements every
# transform directly from the standard formulas (no third-party crate, no model
...
# runtime ships. `git` is preinstalled in the rust image (needed if the
# workspace ever pins `vgi` via a git dependency).
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin color-worker \
    && cp target/release/color-worker /color-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-color
...
LABEL org.opencontainers.image.title="vgi-color" \
      org.opencontainers.image.description="Color science (color-space conversions, CIEDE2000 ΔE, WCAG contrast) as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_COLOR_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /color-worker /usr/local/bin/color-worker
