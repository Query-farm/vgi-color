#!/usr/bin/env bash
# Build the browser (`worker:`) build of the color VGI worker: color-wasm
# compiled to wasm32-unknown-emscripten, linked by emcc with the SAB ring
# js-library into a MODULARIZE'd Web Worker module.
#
...
#     recompiles compiler_builtins with atomics. Also needs `rust-src`:
...
LIB="target/$TARGET/release/libcolor_wasm.a"
...
# threads convoy on it; mimalloc is thread-caching and scales like native.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten defaults to a 64 KiB stack,
# which the Arrow encoders can overflow (surfacing as a bare "memory access out
# of bounds"). INITIAL_MEMORY must cover main stack + every pthread stack up
