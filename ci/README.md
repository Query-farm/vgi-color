# CI: the vgi-color worker integration suite
...
(`test/sql/*.test`) against the vgi-color VGI worker through the **real DuckDB
...
| `TRANSPORT`  | `VGI_COLOR_WORKER` (the ATTACH LOCATION) | how the worker is launched |
|--------------|------------------------------------------|----------------------------|
| `subprocess` | `…/target/release/color-worker`          | DuckDB spawns the stdio binary (default) |
| `http`       | `http://127.0.0.1:<port>`                | `color-worker --http` (auto port; prints `PORT:<n>` on stdout, which the script polls for) |
| `unix`       | `unix:///tmp/color.<pid>.sock`           | `color-worker --unix <sock>` (prints `UNIX:<sock>` on stdout + creates the socket; the script waits for both) |
...
1. **Build the worker** — `cargo build --release --bin color-worker`. The
   compiled `target/release/color-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_COLOR_WORKER}`).
...
3. **Preprocess** — the standalone runner links none of the extensions the
   ...
   through untouched. (The vgi-color `.test` files already `LOAD vgi;`
   explicitly and use `require-env VGI_COLOR_WORKER`, so the `require`-rewrite is
...
4. **Run** — [`run-integration.sh`](run-integration.sh) brings up the worker for
   the selected `TRANSPORT` and sets `VGI_COLOR_WORKER` accordingly, stages the
...
```bash
cargo build --release --bin color-worker
...
WORKER_BIN="$PWD/target/release/color-worker" \
TRANSPORT=subprocess \
  ci/run-integration.sh

# HTTP leg: launches `color-worker --http`, reads PORT:<n>, attaches http://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/color-worker" \
  TRANSPORT=http ci/run-integration.sh

# Unix leg: launches `color-worker --unix /tmp/color.<pid>.sock`, attaches unix://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/color-worker" \
