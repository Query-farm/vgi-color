...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-color
...
# is what LOCATION (VGI_COLOR_WORKER) the .test files ATTACH:
#
#   subprocess  VGI_COLOR_WORKER = the stdio worker command (DuckDB spawns it).
#   http        start `color-worker --http` (auto port; advertises `PORT:<n>`
#               on stdout), VGI_COLOR_WORKER = http://127.0.0.1:<port>.
#   unix        start `color-worker --unix <sock>` (advertises `UNIX:<sock>`
#               on stdout), VGI_COLOR_WORKER = unix://<sock>.
#
# Required environment:
#   HAYBARN_UNITTEST  path to the haybarn-unittest binary
#   WORKER_BIN        path to the compiled color-worker binary (used to launch
#                     the http/unix servers, and the stdio LOCATION). Falls back
#                     to VGI_COLOR_WORKER when that is a bare command (subprocess).
# Optional:
...
WORKER_BIN="${WORKER_BIN:-$REPO/target/release/color-worker}"
...
# Bring up the out-of-band server for http/unix and resolve VGI_COLOR_WORKER.
...
start_server_and_set_location() {
  local kind="$1"
  : "${WORKER_BIN:?path to the color-worker binary (WORKER_BIN)}"
  ...
  case "$kind" in
    http)
      ...
      export VGI_COLOR_WORKER="http://127.0.0.1:$port"
      echo "HTTP worker ready on 127.0.0.1:$port (pid $SERVER_PID)"
      ;;
    unix)
      SOCK_PATH="${VGI_COLOR_SOCK:-/tmp/color.$$.sock}"
      ...
      export VGI_COLOR_WORKER="unix://$SOCK_PATH"
...
: "${VGI_COLOR_WORKER:?worker LOCATION (stdio command, http:// URL, or unix:// socket)}"
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_COLOR_WORKER) ..."
...
if grep -qiE "All tests were skipped|total skipped [1-9]" "$REPORT"; then
  ...
  echo "       is NOT a pass. Transport=$TRANSPORT worker=$VGI_COLOR_WORKER." >&2
  exit 1
fi
exit "$status"
