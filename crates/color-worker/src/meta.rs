...
const SOURCE_BASE: &str =
    "https://github.com/Query-farm/vgi-color/blob/main/crates/color-worker/src";

/// Build the implementation `vgi.source_url` for a file under `color-worker/src`,
/// e.g. `source_url("scalar/convert.rs")`.
...
///
/// `relative_path` is the implementing file relative to `color-worker/src`.
pub fn object_tags(
    title: &str,
    description_llm: &str,
    description_md: &str,
